...
function MessagingCenter({ user, onClose, initialBusiness, embedded = false }) {
  ...
  // token; an anonymous guest is scoped by a browser-generated guest token
  // (localStorage, see getGuestToken) passed with every messaging request.
  const isStaff = user?.accountType === "staff";
  const canHoldConversation = !isStaff;
  const guestToken = user ? null : getGuestToken();
  const selfSenderType = !user ? "guest" : user.accountType === "business_owner" ? "business_owner" : "customer";
  const { data: conversationsData, refetch } = useMyConversations(canHoldConversation, guestToken);
  ...
  const sendMessage = async () => {
    ...
    setActionError(null);
    try {
      // A guest's requests carry their browser guest token — that's the
      // whole credential; no sign-in is ever required to talk to support.
      const guestFields = guestToken ? { guest_token: guestToken } : {};
      if (activeConv) {
          ...
          {(
            <>
              ...
              {/* Composer — guests get the same real input as customers/
                  business owners and their messages send for real (scoped by
                  their browser guest token — no account required). Only a
                  staff session loses the composer — its inbox is the admin
              ...
              <div style={{padding:"12px 14px",borderTop:"1px solid #f0f0f0",background:"white"}}>
                ...
                <div style={{fontSize:"0.6rem",color:"#aaa",marginTop:6,textAlign:"center"}}>
                  {user
                    ...
                    : "Chatting as Guest — your conversation is saved to this browser; create an account to keep it on all your devices"}
                </div>
              </div>
              ) : (
...
// ─── 404 Not Found Page ───────────────────────────────────────────────────────
function NotFoundPage({ onHome, user }) {
  return (
    <div style={{fontFamily:"'Georgia',serif",background:C.cream,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{textAlign:"center",maxWidth:400}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          {user && <a href="https://wa.me/233244000000?text=Hello AshantiHub, I need help." target="_blank" rel="noopener noreferrer"
...
export default function AshantiHub() {
  ...
  //
  // Enabled for everyone except a staff session (whose inbox is the admin
  // MessagingPanel — this endpoint 403s staff by design; a !!user gate here
  // once fired 3 React Query retries on every staff dashboard load). Guests
  // are scoped by their browser guest token, same as MessagingCenter's own
  // call, so the badge works for them too and the query key/cache is shared.
  // Waits for session restore (auth.isLoading) to settle first — before
  // that, `user` is still null for everyone, and a staff session would
  // transiently fire a pointless guest-token fetch.
  const { data: myConversationsData } = useMyConversations(
    !auth.isLoading && user?.accountType !== "staff",
  ...
  return (
    <div style={{fontFamily:"'Georgia',serif",background:C.cream,minHeight:"100vh"}}>
          ...
          <div style={{maxWidth:1280,margin:"0 auto",padding:"16px 14px 40px",display:"flex",gap:20,alignItems:"flex-start"}}>
            ...
            {filters.category==="grocery"?(
              <div style={{background:"white",borderRadius:16,padding:"24px",textAlign:"center",boxShadow:"0 2px 12px rgba(0,0,0,0.07)"}}>
                ...
                {/* WhatsApp ordering is an account-holder channel; a guest
                ...
                <button onClick={()=>setAuthModal("signup")} style={{marginTop:16,background:C.gold,color:C.darkBrown,border:"none",borderRadius:30,padding:"11px 24px",fontWeight:900,cursor:"pointer",fontFamily:"inherit",fontSize:"0.85rem"}}>
      ...
      {/* Floating WhatsApp — an account-holder channel only; guests use the
          in-app chat (ChatLauncher above, open to everyone) instead. */}
      {user && <div onClick={()=>window.open("https://wa.me/233244000000","_blank")}
